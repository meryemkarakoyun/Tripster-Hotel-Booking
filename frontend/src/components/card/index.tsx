...
const Card: FC<Props> = ({ place }) => {
  return (
    <Link
      to={`/place/${place.id}`}
      className="border border-zinc-200 rounded-xl p-5 gap-5 grid grid-cols-3 min-h-75 bg-white card-hover group overflow-hidden"
    >
      ...
      <div className="col-span-2 flex flex-col justify-between">
        <div className="flex flex-col gap-3">
          ...
          <p className="text-zinc-600 flex items-center gap-1">
            <MapPin className="size-4" /> {place.location}
          </p>
